...
def select_type
  ...
  loop do
    ...
    when 2
      ...
      puts 'Неверно задан тип. Выбери 1 или 2'
    end
...
def assign_route
  ...
  if @train.train_route.nil?
    ...
    puts "Ошибка! У поезда #{@train.number} уже есть маршрут #{@train.train_route}!"
  end
end
...
def hook_wagon
  ...
  if wagon.current_train.nil?
    if train.type == wagon.type
      ...
      puts "Ошибка! Поезд #{train.type} и вагон #{wagon.type} несовместимы!"
    end
  else 
    puts "Ошибка! Вагон уже прицеплен к поезду #{wagon.current_train}"
  end
end
