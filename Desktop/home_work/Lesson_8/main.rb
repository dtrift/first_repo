...
def create_route
  ...
  puts "Создан маршрут #{name} - начальная станция #{first_station.name},
        конечная станция #{last_station.name}"
end
...
def list_route_stations
  @route.all_stations.each.with_index do |_station, index|
...
def busy_wagon
  ...
  if wagon.type == :cargo
    ...
    puts "Вагон #{wagon.number} #{wagon.type}, cвободных мест в вагоне #{wagon.free_seats}"
  end
end
...
def send_train
  ...
  loop do
    ...
    when 1
      train.move_forward
      puts "Предыдущая станция - #{train.previous_station.name}" unless train.previous_station.nil?
      puts "Текущая станция - #{train.current_station.name}"
...
def list_wagons
  @wagons.each.with_index do |_wagon, index|
    puts "#{index + 1}. Вагон номер #{@wagons[index].number} #{@wagons[index].type}
          => поезд: #{@wagons[index].current_train}"
  end
end
    ...
    station.show_all_trains do |train|
      ...
      train.show_all_wagons do |wagon|
        if wagon.type == :passenger
          puts "Номер вагона - #{wagon.number}, тип - #{wagon.type}, количество мест - #{wagon.seats},
                занятых мест - #{wagon.busy_seats}, свободных мест - #{wagon.free_seats}"
        else
          puts "Номер вагона - #{wagon.number}, тип - #{wagon.type}, объем - #{wagon.volume},
                занятый объем - #{wagon.busy_volume}, свободный объем - #{wagon.free_volume}"
        end
      end
    end
  end
